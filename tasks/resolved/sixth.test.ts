...
import { SettingsService } from './sixth.ts';
...
describe('SettingsService', () => {
  ...
  beforeEach(() => {
    ...
    lsMock = {
      ...
      getItem: vi.fn(key => {
        return lsMock._value
          ? JSON.parse(lsMock._value[key])
          : null;
      })
    }
  ...
  test('При вызове saveTheme("dark") должен быть вызван localStorage.setItem с правильным ключом и JSON-строкой', () => {
    ...
    expect(lsMock.setItem).toHaveBeenCalledWith(KEY, JSON.stringify({ theme: theme.dark }));
  })
